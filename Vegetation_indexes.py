...
def calculate_EXGreen(img):
    arr = np.asarray(img).astype(float) 
    EXG_array = 2*arr[:,:,1]-arr[:,:,0]-arr[:,:,2]             
    EXG_array[np.isnan(EXG_array)] = 0  
    return EXG_array
...
def create_images(img_path,color,index_name): 
    ...
    elif index_name=="EXG":array=calculate_EXGreen(img)  
    ...
    if index_name=="EXG": 
        if color=="gray":save_path=f"{EXGreen_IMAGES_GRAY}/{img_path.stem}_grayscale{img_path.suffix}" 
        else:save_path=f"{EXGreen_IMAGES_COLOR}/{img_path.stem}_colormap{img_path.suffix}"  
